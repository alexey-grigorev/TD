...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        for c in classes:
            ...
            # filter
            if FILTER:
                dects = Evaluator.filterBboxes(dects)
            ...
            ret.append(r)
            if RETURN_ERRORS:
                fp_bboxes = np.array(dects)[np.where(FP==1)[0]]
            
        if RETURN_ERRORS:
            assert len(classes) == 1
            return ret, fp_bboxes
        else:
            return ret
    ...
    @staticmethod
    def filterBboxes(dects):
        dects = np.array(dects)
        images = np.unique(dects[:,0])
        filtered_dects = []
        for image in images:
            image_dects = dects[np.where(dects[:,0]==image)[0]]
            image_dects = image_dects[image_dects[:,2].argsort()]
            mask = np.zeros(len(image_dects))
            for i, dect_1 in enumerate(image_dects[:-1]):
                total_fine = 0
                for dect_2 in image_dects[i+1:]:
                    prob_2 = dect_2[2]
                    iofirst = Evaluator.iofirst(dect_1[3], dect_2[3])
                    if prob_2 > 0.95:
                        total_fine += iofirst
                if total_fine > 0.7:
                    mask[i] = 1
            filtered_dects += image_dects[np.where(mask==0)[0]].tolist()
        return filtered_dects
    
    @staticmethod
    def iofirst(boxA, boxB):
        # if boxes dont intersect
        if Evaluator._boxesIntersect(boxA, boxB) is False:
            return 0
        interArea = Evaluator._getIntersectionArea(boxA, boxB)
        area_A = Evaluator._getArea(boxA)
        # intersection over area of 1st bbox
        iofirst = interArea / area_A
        assert iofirst >= 0
        return iofirst
